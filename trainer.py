...
from vmap import performance_measure
...
class Trainer:
    def __init__(self, cfg):
        ...
        if self.obj_id == 0:
            self.bound_extent = 0.98
        else:
            self.bound_extent = 0.85
    ...
    def meshing(self, bound, obj_center, grid_dims=(256, 256, 256)):
        with performance_measure(f"Calculate grid {grid_dims}"):
            occ_range = [-1., 1.]
            ...
            grid_pc = render_rays.make_3D_grid(occ_range=occ_range, dims=grid_dims, device=self.device,
                                            scale=scene_scale, transform=transform).view(-1, 3)
            grid_pc -= obj_center.to(grid_pc.device)
        with performance_measure(f"1st nn call, {grid_pc.shape}"):
            ret = self.eval_points(grid_pc)
            if ret is None:
                return None

            occ, _ = ret
        with performance_measure("Marching cube"):
            mesh = vis.marching_cubes(occ.view(*grid_dims).cpu().numpy())
            if mesh is None:
            ...
            # dim_scale =  np.array(grid_dims) / np.array(grid_dims).max()
            # print(grid_dims, dim_scale)
            # mesh.apply_scale(dim_scale)
        ...
        with performance_measure(f"2nd nn call {vertices_pts.shape}"):
    ...
    def eval_points(self, points, chunk_size=100000):
        ...
        with torch.no_grad():
            for k in range(n_chunks): # 2s/it 1000000 pts
